...
if(isset($_POST["submit"])) {
    ...
    if(empty($nama)){
        ...
        echo "<script>alert('no telepon pelanggan tidak boleh kosong');location.href='pelanggan.php';</script>"; 
    } else {
        if(empty($foto)) {
                
            $query = "UPDATE `pelanggan` SET `nama_pelanggan`='$nama',`username`='$username',`alamat`='$alamat',`telp`='$telp' WHERE `id_pelanggan`='$id_pelanggan'";
                
            $insert=mysqli_query($conn, $query);

            if($insert) {
                echo "<script>alert('Sukses mengubah pelanggan');location.href='pelanggan.php';</script>";
            } else {
                echo "<script>alert('Gagal mengubah pelanggan');location.href='pelanggan.php';</script>";
            }
        }
        ...
        // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                ...
                $sql = "UPDATE `pelanggan` SET `nama_pelanggan`='$nama',`username`='$username',`alamat`='$alamat',`telp`='$telp',`foto_pelanggan`='$foto' WHERE `id_pelanggan`='$id_pelanggan'";
